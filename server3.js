...
app.get('/', async (req, res) => {
    console.log(`S3 TG2 - Received a get request at ${moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:MM:SS')}. Path: /`);
    res.send('Hello World!');
});

app.get('/hello', async (req, res) => {
    console.log(`S3 TG2 - Received a get request at ${moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:MM:SS')}. Path: /hello`);
    res.send('Hello World!');
});
...
io.on("connection", (socket) => {
    console.log(`S3 TG2 - Client connected [id=${socket.id}] at ${moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:MM:SS')}`);
    ...
    socket.on("disconnect", () => {
        ...
        console.log(`S3 TG2 - Client [id=${socket.id}] disconnected at ${moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:MM:SS')}`);
    });

    console.log(`S3 TG2 - Outside getUserDetails Event - ${moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:MM:SS')}`);
    ...
    socket.on("getUserDetails", (params = {}) => {
        ...
        console.log(`S3 TG2 - Inside getUserDetails Event - ${moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:MM:SS')}`);
        let message;
        if (!requiredUser) {
        ...
        console.log(`S3 TG2 - Just Before Emit Event - ${moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:MM:SS')}`);
