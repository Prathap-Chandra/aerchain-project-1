...
app.get('/', async (req, res) => {
    console.log(`S4 TG2 - Received a get request at ${new Date().toLocaleString()}. Path: /`);
    res.send('Hello World!');
});

app.get('/hello', async (req, res) => {
    console.log(`S4 TG2 - Received a get request at ${new Date().toLocaleString()}. Path: /hello`);
    res.send('Hello World!');
});
...
io.on("connection", (socket) => {
    console.log(`S4 TG2 - Client connected [id=${socket.id}] at ${new Date().toLocaleString()}`);
    ...
    socket.on("disconnect", () => {
        ...
        console.log(`S4 TG2 - Client [id=${socket.id}] disconnected at ${new Date().toLocaleString()}`);
    });

    console.log(`S4 TG2 - Outside getUserDetails Event - ${new Date().toLocaleString()}`);
    ...
    socket.on("getUserDetails", (params = {}) => {
        ...
        console.log(`S4 TG2 - Inside getUserDetails Event - ${new Date().toLocaleString()}`);
        let message;
        if (!requiredUser) {
        ...
        console.log(`S4 TG2 - Just Before Emit Event - ${new Date().toLocaleString()}`);
