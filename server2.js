...
app.get('/', async (req, res) => {
    console.log(`S2 TG1 - Received a get request at ${moment().tz('Asia/Kolkata').toDate()}. Path: /`);
    res.send('Hello World!');
});

app.get('/hello', async (req, res) => {
    console.log(`S2 TG1 - Received a get request at ${moment().tz('Asia/Kolkata').toDate()}. Path: /hello`);
    res.send('Hello World!');
});
...
io.on("connection", (socket) => {
    console.log(`S2 TG1 - Client connected [id=${socket.id}] at ${moment().tz('Asia/Kolkata').toDate()}`);
    ...
    socket.on("disconnect", () => {
        ...
        console.log(`S2 TG1 - Client [id=${socket.id}] disconnected at ${moment().tz('Asia/Kolkata').toDate()}`);
    });

    console.log(`S2 TG1 - Outside getUserDetails Event - ${moment().tz('Asia/Kolkata').toDate()}`);
    ...
    socket.on("getUserDetails", (params = {}) => {
        ...
        console.log(`S2 TG1 - Inside getUserDetails Event - ${moment().tz('Asia/Kolkata').toDate()}`);
        let message;
        if (!requiredUser) {
        ...
        console.log(`S2 TG1 - Just Before Emit Event - ${moment().tz('Asia/Kolkata').toDate()}`);
